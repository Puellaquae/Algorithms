...
void fly_x() {
    ...
    for (int i = 1; i <= N; i++) {
        obj_array.push_back(
            Object(wall[i].x1, wall[i].y1, wall[i].y2, i, WALL));
        if (wall[i].x1 != wall[i].x2) {
            obj_array.push_back(
                Object(wall[i].x2, wall[i].y1, wall[i].y2, i, WALL));
        }
    }
...
void fly_y() {
    ...
    for (int i = 1; i <= N; i++) {
        obj_array.push_back(
            Object(wall[i].y1, wall[i].x1, wall[i].x2, i, WALL));
        if (wall[i].y1 != wall[i].y2) {
            obj_array.push_back(
                Object(wall[i].y2, wall[i].x1, wall[i].x2, i, WALL));
        }
    }
...
int main() {
    ...
    for (int i = 1; i <= N; i++) {
        wall[i].x1 = lower_bound(listx.begin(), listx.end(), wall[i].x1) -
                     listx.begin() + 1;
        wall[i].x2 = lower_bound(listx.begin(), listx.end(), wall[i].x2) -
                     listx.begin() + 1;
        wall[i].y1 = lower_bound(listy.begin(), listy.end(), wall[i].y1) -
                     listy.begin() + 1;
        wall[i].y2 = lower_bound(listy.begin(), listy.end(), wall[i].y2) -
                     listy.begin() + 1;
    }
    for (int i = 1; i <= M; i++) {
        bird[i].x = lower_bound(listx.begin(), listx.end(), bird[i].x) -
                    listx.begin() + 1;
        bird[i].y = lower_bound(listy.begin(), listy.end(), bird[i].y) -
                    listy.begin() + 1;
    }
